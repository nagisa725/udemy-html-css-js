document.addEventListener ('DOMContentLoaded', function () {
    ...
    //'.animate-title'全てを取得したいのでAllを使用しelsに格納
...
const io = new IntersectionObserver(cb, options);
    /*new演算子 複数のプログラムが同じオブジェクトに対し干渉しバグが起きないためにオブジェクト本体からコピーし処理を実行するためのもの*/ 
    /*IntersectionObserver(交差オブサーバー) 指定した範囲(root)に、
    監視している対象(observe())の要素が入ってきたら(交差したら)その要素に対して処理を実行できる*/ 
els.forEach(el => io.observe(els));
//elsに格納されている値がforEachで一つずつ処理される 複数処理のため関数使用
